...
- (void)parseXMLWithUrlLocal:(NSURL *)xmlUrl
								...
								basePathOrNil:(NSIndexPath *)path
{
	...
	[reader parseXMLWithUrlLocal:xmlUrl
					rootFolderId:[rootId stringValue]
					  parseError:error
					 folderModel:folder
				   basePathOrNil:path];
	[reader release];
...
+ (BoxFolder *)folderForId:(NSNumber *)rootId
			 ...
			 basePathOrNil:(NSIndexPath *)path
{
	NSString *url = [BoxRESTApiFactory getAccountTreeOneLevelUrlString:token
														   boxFolderId:[NSString stringWithFormat:
																		@"%@",
																		rootId]];
