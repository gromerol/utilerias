...
def compile_js(manifest,config):
	js_file = os.path.join(cwd,'assets','ti.bump.js')
	...
	f = os.path.join(cwd,'Classes','TiBumpModuleAssets.m')
	c = open(f).read()
	idx = c.find('return ')
	before = c[0:idx]
	after = """
...
ignoreFiles = ['.DS_Store','.gitignore','libTitanium.a','titanium.jar','README','ti.bump.js']
...
def package_module(manifest,mf,config):
	...
	for dn in ('assets','example', 'Resources'):
