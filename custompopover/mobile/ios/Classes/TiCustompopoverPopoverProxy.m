...
#import "TiCustompopoverPopoverProxy.h"
#import "TiCustompopoverPopover.h"
#import "TiCustompopoverLayoutEntry.h"
#import "TiUtils.h"
#import <libkern/OSAtomic.h>

static TiCustompopoverPopoverProxy * currentlyDisplaying = nil;

@implementation TiCustompopoverPopoverProxy
...
{
	NSLog(@"Hello? World? %@",properties);
	[super _initWithProperties:properties];
	if ([TiCustompopoverLayoutEntry layoutForKey:nil]==nil) {
		TiCustompopoverLayoutEntry * newEntry = [[TiCustompopoverLayoutEntry alloc] init];
		[newEntry setArrowOverlap:10.0];
		[newEntry setBorderMargins:UIEdgeInsetsMake(20,20,20,20)];
		[newEntry setBorderArrowUp:[TiUtils image:@"/popoverUpArrow.png" proxy:self]];
		[newEntry setBorderArrowLeft:[TiUtils image:@"/popoverLeftArrow.png" proxy:self]];
		[newEntry setBorderArrowRight:[TiUtils image:@"/popoverRightArrow.png" proxy:self]];
		[newEntry setBorderArrowDown:[TiUtils image:@"/popoverDownArrow.png" proxy:self]];
		[newEntry setBorderBackground:[TiUtils stretchableImage:@"/popoverBackground.png" proxy:self]];
		[TiCustompopoverLayoutEntry registerLayout:newEntry forKey:nil];
	}
}
...
-(UIPopoverController *)popoverController
{
	if (popoverController == nil)
	{
		...
		NSLog(@"Starting popoverController");
		if ([popoverController respondsToSelector:@selector(setPopoverBackgroundViewClass:)]) {
			NSLog(@"Looking up class");
			Class customClass = NSClassFromString(@"TiCustompopoverBackgroundView");
			NSLog(@"Found class %@",customClass);
			if (customClass != nil) {
